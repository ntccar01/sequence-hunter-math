const lessons = [
  ...
  },
  {
    ...
    tag: "第 1 節",
    title: "遞迴數列入門",
    subtitle: "3-3 初版｜前一項、下一項、依規則往後推",
    goals: [
      "能說出遞迴數列是用前面的項推下一項。",
      "能分辨首項與遞迴規則。",
      "能依遞迴規則寫出前幾項。",
    ...
    guideIntro: "3-3 先放初版遞迴數列入門。這節先讓學生知道：有些數列不是一開始就給一般項，而是給首項和「下一項怎麼來」的規則。",
    guideBlocks: [
      {
        title: "先看接力規則",
        example: "前一項 + 3",
        talk: "請學生想像接力：下一棒要接著前一棒的結果走，不是每次都重新開始。",
        feature: true
      },
      {
        title: "首項是起點",
        example: "a1=2",
        talk: "沒有起點就推不下去。先圈出第一項，再看規則。"
      },
      {
        title: "下一項怎麼來",
        example: "a(n+1)=a(n)+3",
        talk: "這句話的意思是：下一項等於前一項加 3。先用口語講，不急著背符號。"
      },
      {
        title: "一步一步推",
        example: "2 → 5 → 8",
        talk: "每次只做一步：知道第 1 項，才能推第 2 項；知道第 2 項，才能推第 3 項。"
      },
      {
        title: "乘法遞迴",
        example: "前一項 × 2",
        talk: "遞迴規則不一定是加，也可能是乘。重點是都要看前一項。"
      },
      {
        title: "兩項一起看",
        example: "前兩項相加",
        talk: "有些規則會用到前兩項，像 1,1,2,3,5。這類要先找前兩個起點。"
      },
      {
        title: "符號翻譯",
        example: "a4 由 a3 推出",
        talk: "看到 a4 時，提醒學生它通常由前面的 a3 或 a2、a3 算出來。"
      },
      {
        title: "先列表再作答",
        example: "第1項到第5項",
        talk: "遞迴題最適合畫表格，避免跳步造成錯誤。"
      }
    ],
    timeline: [
      ["0-5", "登入與回顧", "回顧等差、等比都是有規則的數列，今天換成用前一項往後推。"],
      ["5-12", "生活接力導入", "用階梯、存錢、遊戲血量回復說明下一項接著前一項。"],
      ["12-22", "首項與規則", "圈出 a1 和遞迴規則，口語翻譯為下一項怎麼來。"],
      ["22-31", "老師示範", "用 a1=4，下一項=前一項+5，示範列前 5 項。"],
      ["31-37", "兩項規則預告", "用 1,1,2,3,5 簡單說明有些規則會看前兩項。"],
      ["37-42", "投放學生任務", "學生完成遞迴入門章節，先在學習單列出表格再送答案。"],
      ["42-45", "冷卻整理", "學生寫一句：遞迴就是用前面的項推出後面的項。"]
    ],
    demoQuestion: "已知 a1=4，且下一項都比前一項多 5，請寫出前 5 項。",
    demoSteps: [
      "先圈起點：a1=4。",
      "把規則翻成口語：下一項=前一項+5。",
      "第 2 項：4+5=9。",
      "第 3 項由第 2 項推出：9+5=14。",
      "繼續推到第 5 項，最後請學生自己完成。"
    ],
    observe: [
      "學生是否先找首項再開始推。",
      "學生是否把遞迴規則誤當成一般項公式。",
      "學生是否能一格一格往後寫，不跳步。",
      "學生遇到前兩項規則時是否知道要先有兩個起點。"
    ],
    studentTask: "學生端完成「遞迴數列入門」章節：辨認遞迴、依規則推項、前兩項相加與生活情境。",
    checklist: [
      "戰情室確認學生是否能完成前 5 項推算。",
      "若錯題集中在符號，下一節先補 a(n+1) 與 a(n) 的意思。",
      "若跳步錯很多，要求學習單一定先畫表格。"
    ]
  }
];
...
function getLessonChapterId(lesson) {
  if (lesson.id.startsWith("lesson-3-3")) return "3-3";
